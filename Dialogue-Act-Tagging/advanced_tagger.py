...
for dialogue in master_list_input:
    for utterance in dialogue:
        ...
        if utterance.pos is None:
            ...
            for ind, tup in enumerate(utterance.pos):
                token, pos = tup
                utterance_vector.append("TOKEN_" + token)
                utterance_vector.append("POS_" + pos)
                if ind == 0:
                    utterance_vector.append(token + "|BOS")
                    utterance_vector.append(pos + "|BOS")
                if ind == len(utterance.pos) - 1 :
                    utterance_vector.append("EOS|" + token)
                    utterance_vector.append("EOS|" + pos)
                if not ind == 0:
                    prevtok, prevpos = utterance.pos[ind - 1]
                    utterance_vector.append(token + "|" + prevtok)
                    utterance_vector.append(pos + "|" + prevpos)
